...
def sympy_to_cvc5(expr, var):
  ...
  match expr:
    ...
    case sp.GreaterThan():
      l, r = [sympy_to_cvc5(arg, var) for arg in expr.args]
      return l >= r
    case sp.LessThan():
      l, r = [sympy_to_cvc5(arg, var) for arg in expr.args]
      return l <= r
...
class Verifier_Reach_ABV_CVC5(Verifier_Reach_ABV):
  def chk_dec(self):
    ...
    x_sp = X(dim)
    y_sp = Y(dim)
    x_c5 = cvc5.RealVector('x', dim)
    y_c5 = cvc5.RealVector('y', dim)
    var = {x_sp[i]: x_c5[i] for i in range(dim)}
    var.update({y_sp[i]: y_c5[i] for i in range(dim)})

    bounds, problem = [], []
    bounds.append(BoundIn(self.env.bnd, x_sp))
    bounds.append(BoundOut(self.env.tgt, x_sp))
    bounds = [sympy_to_cvc5(b, var) for b in bounds]

    # ||y||_1 <= B(x)
    norm_y = sympy_to_cvc5(Norm_L1(y_sp), var)
    b = sympy_to_cvc5(Net(self.B, x_sp)[0], var)
    problem.append(norm_y <= b)

    # V(x) <= V( A(x) + y) )
    v = sympy_to_cvc5(Net(self.V, x_sp)[0], var)
    vp = sympy_to_cvc5(
      Net(self.V, Net(self.A, x_sp) + y_sp)[0], var)
    problem.append(v <= vp)
    # logger.debug('bounds=' + pprint.pformat(bounds))
    # logger.debug('problem=' + pprint.pformat(problem))
    return solve_cvc5(bounds + problem, x_c5)